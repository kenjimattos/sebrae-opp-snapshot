...
- **Scaffold da API de leitura** (`server/`) — processo Node/Fastify isolado que lê o MongoDB `DadosOPP` e devolve o mesmo shape (`IndicatorsData`) que o frontend consumia dos TS estáticos. Roda na máquina da app (`<host-da-app>`), lê o banco (`<host-do-banco>`), Nginx faz proxy de `/api/*`.
  ...
  - Validado ao vivo contra o `DadosOPP` (<host-do-banco>): 223 municípios, semáforo calculado no servidor (IGM-CFA 7,06→warning etc.), mapa com 22 opções (os 2 não-implementados não têm doc no banco, então não são retornados).
