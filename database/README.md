...
>   (`<host-da-app>`): Nginx serve o `dist/` e faz proxy de `/api/*` para o processo Node.
...
e emite os dois seeds. **Roda na `<host-do-banco>`** (única máquina que vê o lake): topologia,
...
# direto do lake + escreve no Mongo OPP (rodar na <host-do-banco>):
python3 database/scripts/gerar_seed_escolaridade.py \
  ...
  --opp-user <usuario-opp> --opp-pass 'SENHA' --write-mongo
```
...
Sebrae** (base `IDEB`/`<usuario-lake>`, INEP — não basedosdados) — um único script lê a série de
edições por município e emite os **dois seeds**, separados pelo campo `SERIE` do dump. **Roda na
`<host-do-banco>`** (única máquina que vê o lake): credenciais, pré-requisitos (Python 3.6,
...
# 2) gerar os dois seeds + snapshot (defaults: host do lake + <usuario-lake>):
...
> como `<usuario-lake>`, ajuste via `--rede` / `IDEB_MONGO_DB` (ou o bloco `IDEB_*` do `.env`). Se a
...
O card `remuneracao-media` vem **direto do data lake do Sebrae** (base `RAIS`/`<usuario-lake>`, a
...
`<host-do-banco>`** (única máquina que vê o lake): Python 3.6, `pymongo<4`, cron em **`RUNBOOK_ETL.md`**.
