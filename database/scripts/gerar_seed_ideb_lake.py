...
(base **IDEB**, usuário `<usuario-lake>`), NÃO de API/BigQuery.
...
def _pick_collection(args):
    ...
    if not nomes:
        sys.exit("Banco {!r} não tem coleções (host {}). Confira IDEB_MONGO_DB (a base pode "
                 "se chamar '<usuario-lake>').".format(args.mongo_db, args.mongo_host))
    if args.collection:
...
def _inspect(args):
    ...
    print("  {}".format(", ".join(sorted(nomes)) or "(nenhuma — confira IDEB_MONGO_DB; pode ser '<usuario-lake>')"))
    if not nomes:
        return
    coll = _coll(args)
...
def main():
    global REDE
    load_dotenv()
    ap = argparse.ArgumentParser(description="Seeds do IDEB (anos iniciais + finais) via lake <usuario-lake>.")
