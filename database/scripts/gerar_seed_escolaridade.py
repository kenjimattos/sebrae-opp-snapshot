...
Uso:
  ...
  ssh -L 27017:localhost:27017 <usuario-lake>@HOST_DO_SEBRAE
  ...
  python3 database/scripts/gerar_seed_escolaridade.py --inspect \
      --mongo-user <usuario-lake> --mongo-pass '***' --collection 2024_VINC
...
from typing import List  # 3.6: o host do ETL (<host-do-banco>) só tem Python 3.6, sem `list[...]`
...
def write_mongo(args, indicadores: List[dict], values: List[dict]) -> None:
    ...
    chave dos seeds .js — idempotente. Roda na <host-do-banco>, que vê o OPP local."""
...
def main() -> None:
    ...
    # destino OPP (--write-mongo): Mongo OPP, local na <host-do-banco>
    ...
    ap.add_argument("--opp-db", default=env("OPP_MONGO_DB", "DadosOPP"))  # banco OPP na <host-do-banco> (authSource = mesmo nome)
