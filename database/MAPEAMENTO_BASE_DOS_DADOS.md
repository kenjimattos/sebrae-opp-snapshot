...
- **SL** — Sebrae Data Lake (Ambiente Colaborativo de Dados SEBRAE-PR, Mongo `<host-do-lake>`).
...
RF do lake** — sem BigQuery, sem internet, **uma rodada só** na <host-do-banco>.
...
do `OPP_MONGO_USER`/`OPP_MONGO_PASS` (o user do OPP, `<usuario-opp>`, não segue o padrão `usr_`).
...
`<host-do-banco>` (única máquina que vê o lake e hospeda o Mongo OPP) e escreve direto no
...
> ✅ **Coletado (jun/2026, RF_ESTABELECIMENTOS_2025):** rodada completa na <host-do-banco>,
...
>
> ✅ **Consolidação do `crescimento-mpe` (jul/2026) — rerun online feito.** Rodada na <host-do-banco>
...
>
> ✅ **Base econômica por porte (jul/2026) — coletado.** Run online na <host-do-banco> (03/jul/2026,
...
O data lake do Sebrae **também expõe uma base `IBGE`** (usuário `<usuario-lake>`; note: o usuário
**não** tem `listCollections` na base `<usuario-lake>`, mas **tem** na base real, que se chama
